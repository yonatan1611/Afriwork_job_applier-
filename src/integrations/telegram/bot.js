...
import { TELEGRAM_BOT_TOKEN, TELEGRAM_POLLING } from '../../config/constants.js';
import { updatePreferenceCategory, removePreferenceKey, getJobPreferences, getAllowedCategories, setScoreThreshold, clearScoreThreshold } from '../../repositories/jobPreferencesRepo.js';
...
import { setUserExperience, clearUserExperience } from '../../repositories/userProfileRepo.js';
...
export const bot = TELEGRAM_BOT_TOKEN ? new TelegramBot(TELEGRAM_BOT_TOKEN, { polling: TELEGRAM_POLLING }) : null;
...
if (bot) {
  ...
  function tokenizeArgs(str) {
    ...
    while ((m = regex.exec(normalized))) {
      if (m[2]) tokens.push(m[2]); else if (m[3]) tokens.push(m[3]);
    }
    return tokens;
  }
  ...
  await bot.sendMessage(chatId, `Current preferences:\n<pre>${escapeHtml(JSON.stringify(prefs, null, 2))}</pre>`, { parse_mode: 'HTML' });
  ...
  if (action === 'help') {
        ...
        await bot.sendMessage(chatId, 'Allowed categories:\n' + getAllowedCategories().join(', '));
        return;
        ...
        if (!sub) {
          ...
          const note = current == null ? `Using default (${envDefault})` : `Current: ${current}`;
          await bot.sendMessage(chatId, `Match score threshold -> ${note}`);
          return;
        }
        if (sub === 'set') {
          const val = Number(parts.shift());
          if (!Number.isFinite(val)) throw new Error('Usage: /pref threshold set <number>');
        ...
        throw new Error('Unknown threshold subcommand. Use: /pref threshold [set <number>|clear]');
      }
      if (action === 'set') {
        ...
        if (!categoryRaw || !keyRaw || !weightStr) throw new Error('Format: /pref set <category> <key> <number>');
        if (!Number.isFinite(weight)) throw new Error('Weight must be a number');
  const updated = await updatePreferenceCategory(category, key, weight) || {};
  const keys = Object.keys(updated || {});
  const normCat = keys.find(k => k.toLowerCase() === category.toLowerCase()) || category;
  const catObj = (updated && updated[normCat]) ? updated[normCat] : (updated[category] || {});
  await bot.sendMessage(chatId, `Updated ${normCat}.${key} = ${weight}\n<pre>${escapeHtml(JSON.stringify(catObj || {}, null, 2))}</pre>`, { parse_mode: 'HTML' });
      } else if (action === 'del') {
        const [category, key] = parts;
        if (!category || !key) throw new Error('Format: /pref del <category> <key>');
  ...
  bot.onText(/^\/jobs$/i, async (msg) => {
    await bot.sendMessage(msg.chat.id, 'Subcommands:\n/jobs clear');
  });
  ...
  bot.onText(/^\/threshold\s+set\s+(\S+)$/i, async (msg, match) => {
    ...
    if (!Number.isFinite(val)) return bot.sendMessage(chatId, 'Usage: /threshold set <number>');
    try {
      await setScoreThreshold(val);
      await bot.sendMessage(chatId, `Threshold updated to ${val}.`);
    } catch (e) {
    ...
    try {
      ...
      if (data.cmd === 'ignore') {
        ...
        if (chatId && msgId) {
          ...
          await bot.editMessageText(newText, { chat_id: chatId, message_id: msgId, parse_mode: 'HTML', reply_markup: { inline_keyboard: [] } });
        }
        ...
        try {
          const letter = await generateCoverLetterForJobId(String(data.job_id), chatId);
          await saveCoverLetter({ job_id: String(data.job_id), job_title: query.message?.text?.match(/<b>💼 Job Title:<\/b> ([^\n]+)/)?.[1] || '', content: letter });
          if (chatId) {
            await bot.sendMessage(chatId, '<b>Cover Letter Draft</b>\n\n' + escapeHtml(letter), { parse_mode: 'HTML' });
          }
          // Apply via Afriwork API (API-only)
          try {
            const username = query.from?.username || undefined;
            await applyViaApi({ jobId: String(data.job_id), coverLetter: letter, telegramUsername: username });
            if (chatId) {
              await bot.sendMessage(chatId, '✅ Application submitted via API.');
            ...
            if (chatId) await bot.sendMessage(chatId, '⚠️ Auto-apply error: ' + (autoErr?.message || String(autoErr)));
          }
        } catch (e) {
          if (chatId) await bot.sendMessage(chatId, 'Failed to generate cover letter. ' + e.message);
        }
      }
    } catch (e) {
      // Fallback: acknowledge to avoid Telegram retry storm
      if (query?.id) await bot.answerCallbackQuery(query.id, { text: 'Action failed' }).catch(() => {});
  ...
  bot.onText(/^\/exp\s+set\s+([\s\S]+)/i, async (msg, match) => {
    ...
    if (!text) return bot.sendMessage(chatId, 'Format: /exp set <your experience text>');
    try {
  ...
  await bot.sendMessage(chatId, '<b>Saved experience</b>\n\n<pre>' + escapeHtml(profile.experience_text) + '</pre>', { parse_mode: 'HTML' });
    } else {
      await bot.sendMessage(chatId, 'No experience saved. Use /exp set <text> to add one.');
    }
  });
