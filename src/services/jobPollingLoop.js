...
async function sendTelegramForMatches(matches) {
  ...
  for (const job of matches) {
    ...
    const compAmount = (typeof job.compensation_amount_cents === 'number' && job.compensation_amount_cents > 0)
      ...
      : 'N/A';
    const message = `<b>💼 Job Title:</b> ${escapeHtml(job.title)}\n` +
    ...
    const inlineKeyboard = {
      inline_keyboard: [
        [
          { text: 'Apply ✅', callback_data: JSON.stringify({ cmd: 'apply', job_id: job.id }) },
          { text: 'Ignore ❌', callback_data: JSON.stringify({ cmd: 'ignore', job_id: job.id }) },
        ],
      ],
    };
